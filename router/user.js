...
router.post("/check", (req, res) => {
    ...
    client.query(sqlStr, [userId], (err, results) => {
        if (err) {
            //查询数据失败
            res.send({
                code: 201,
...
router.post("/add", (req, res) => {
  ...
  //把用户提交的数据保存到数据库
  const sqlStr =
    "insert into user (name, idNum,birthday,gender, email, address,phoneNum) values (?, ?, ?, ?,?,?,?)";
  client.query(
    sqlStr,
    [
      user.name,
...
//   console.log(req.body);
  ...
  //修改用户信息
  const sqlStr =
    "update user set name=?,idNum=?,birthday=?,gender=?,email=?,address=?,phoneNum=? where userId=?";
  client.query(
    sqlStr,
    [
      user.name,
...
router.post("/register", (req, res) => {
  ...
  const sqlStr =
    "insert into user_account (account, password,phoneNumber ,email) values (?, ?, ?, ?)";
  client.query(
    sqlStr,
    [account, password, phoneNumber, email],
    (err, results) => {
      if (err) {
        res.send({
          code: 201,
